...
export class CreateAllTables1708523349210 implements MigrationInterface {
    name = 'CreateAllTables1708523349210'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`planets\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`rotation_period\` int NOT NULL, \`orbital_period\` int NOT NULL, \`diameter\` int NOT NULL, \`climate\` varchar(255) NOT NULL, \`gravity\` varchar(255) NOT NULL, \`terrain\` varchar(255) NOT NULL, \`surface_water\` int NOT NULL, \`population\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`species\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`classification\` varchar(255) NOT NULL, \`designation\` varchar(255) NOT NULL, \`average_heigh\` int NOT NULL, \`skin_colors\` varchar(255) NOT NULL, \`hair_colors\` varchar(255) NOT NULL, \`eye_colors\` varchar(255) NOT NULL, \`average_lifespan\` int NOT NULL, \`language\` varchar(255) NOT NULL, \`planets_id\` int NULL, UNIQUE INDEX \`REL_214c6cd751418b17eead877e30\` (\`planets_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`starships\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`model\` varchar(255) NOT NULL, \`manufacturer\` varchar(255) NOT NULL, \`cost_in_credits\` int NOT NULL, \`length\` int NOT NULL, \`max_atmosphering_speed\` int NOT NULL, \`crew\` int NOT NULL, \`passengers\` int NOT NULL, \`cargo_capacity\` int NOT NULL, \`consumables\` varchar(255) NOT NULL, \`hyperdrive_rating\` int NOT NULL, \`MGLT\` int NOT NULL, \`starship_class\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`films\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`episode_id\` int NOT NULL, \`opening_crawl\` varchar(255) NOT NULL, \`director\` varchar(255) NOT NULL, \`producer\` varchar(255) NOT NULL, \`release_date\` datetime NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`vehicles\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`model\` varchar(255) NOT NULL, \`manufacturer\` varchar(255) NOT NULL, \`cost_in_credits\` int NOT NULL, \`length\` int NOT NULL, \`max_atmosphering_speed\` int NOT NULL, \`crew\` int NOT NULL, \`passengers\` int NOT NULL, \`cargo_capacity\` int NOT NULL, \`consumables\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`peoples_planets\` ADD CONSTRAINT \`FK_614a475f0005af1460a87739045\` FOREIGN KEY (\`planets_id\`) REFERENCES \`planets\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`peoples_species\` ADD CONSTRAINT \`FK_bd47bf66a2593c5d5d094b513dd\` FOREIGN KEY (\`species_id\`) REFERENCES \`species\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`peoples_vehicles\` ADD CONSTRAINT \`FK_4960fe3de3713898f311fa19421\` FOREIGN KEY (\`vehicles_id\`) REFERENCES \`vehicles\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`peoples_starships\` ADD CONSTRAINT \`FK_eae4c1c51971cf3ccced5876b86\` FOREIGN KEY (\`starships_id\`) REFERENCES \`starships\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`films_starships\` ADD CONSTRAINT \`FK_3d33c7a49635895c92e167d376f\` FOREIGN KEY (\`starships_id\`) REFERENCES \`starships\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`films_vehicles\` ADD CONSTRAINT \`FK_37617d1f0a1ddebcc4c4bdb2744\` FOREIGN KEY (\`vehicles_id\`) REFERENCES \`vehicles\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`films_planets\` ADD CONSTRAINT \`FK_9ee75db30fc8e639561bbe45675\` FOREIGN KEY (\`planets_id\`) REFERENCES \`planets\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
